...
2. **履歴が積み重なり、人もAIも育つ** — カードに溜まった人とAIのやり取りが資産になる。AI側は履歴から「働き方のルール」を学び、**言われなくても前提として読んでから動く**ようになる（使うほど成果物が自分にフィットする）。人の側も、AIへの任せ方・指示の出し方・判断の線引きが、やり取りを重ねるほど磨かれていく。**片方が賢くなるのではなく、同じ場で人とAIが互いに育つ**のが Grow の核です。
...
![Grow アーキテクチャ図](docs/images/architecture.png)

> 図のソース（Mermaid）: [`docs/images/architecture.mmd`](docs/images/architecture.mmd)
...
![Grow エージェント編成図](docs/images/agents.png)

> 図のソース（Mermaid）: [`docs/images/agents.mmd`](docs/images/agents.mmd)
...
**テスト状況:** バックエンド 364 tests（pytest）/ フロントエンド 326 tests（vitest）。Mock プロバイダにより DB とスタブAIだけで全フローを決定的に検証できます。
