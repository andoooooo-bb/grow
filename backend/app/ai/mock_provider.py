...
class MockProvider(AiProvider):
    ...
    async def assess_task(self, task: dict, rules: list[dict]) -> AssessResult:
        """受付判定（#27）: title / labels だけで決まる決定的分岐。

        - hearing: addCard 既定タイトル（「新しいタスク」）、または 20 字未満で
          labels 空（判断材料が無い）→ 初期質問（INTAKE_QUESTIONS）を返す
        - breakdown: 「リニューアル」「刷新」等の大きい語を含む、または 30 字超
        - execute: それ以外（「調査」「作成」「まとめ」等の具体的な作業）
        """
        title = str(task.get("title") or "")
        labels = list(task.get("labels") or [])
        route: IntakeRoute
        questions: list[str] = []
        if title == INTAKE_DEFAULT_TITLE or (len(title) < 20 and not labels):
            route = "hearing"
            questions = list(INTAKE_QUESTIONS)
        elif any(keyword in title for keyword in INTAKE_BREAKDOWN_KEYWORDS) or len(title) > 30:
            route = "breakdown"
        else:
            route = "execute"
        reason = INTAKE_REASONS[route]
        return AssessResult(
            route=route,
            questions=questions,
            reason=reason,
            usage=self._usage(f"{route}:{reason}", task, rules),
        )

    async def deep_dive(
        self, task: dict, chat: list[dict], rules: list[dict]
    ) -> DeepDiveResult:
        """深掘り自己判定（#27）: 人の発言回数だけで決まる決定的分岐。

        - 人の発言が 1 回目まで → ask（深掘り質問を 1 つ返し、分解は提案しない）
        - 2 回目以降 → propose（既存 chat_reply / propose_subtasks の合成）
        """
        human_turns = sum(1 for message in chat if message.get("who") == "human")
        if human_turns <= 1:
            return DeepDiveResult(
                mode="ask",
                text=DEEP_DIVE_QUESTION,
                subtasks=[],
                usage=self._usage(DEEP_DIVE_QUESTION, task, chat, rules),
            )
        reply = await self.chat_reply(task, chat, rules)
        proposal = await self.propose_subtasks(task, chat, rules)
        return DeepDiveResult(
            mode="propose",
            text=reply.text,
            subtasks=list(proposal.subtasks),
            usage=TokenUsage(
                input_tokens=reply.usage.input_tokens + proposal.usage.input_tokens,
                output_tokens=reply.usage.output_tokens + proposal.usage.output_tokens,
            ),
        )
...
# addCard の既定タイトル（§5.3）。内容ゼロ = 判定材料が無いので必ずヒアリング
INTAKE_DEFAULT_TITLE = "新しいタスク"

# タイトルにこれらの「大きい語」を含むタスクは分解から始める（決定的キーワード）
INTAKE_BREAKDOWN_KEYWORDS = ("リニューアル", "刷新", "立ち上げ", "再構築")

# hearing 判定の初期質問（intake ジョブが①②③形式で壁打ちに自発投稿する）
INTAKE_QUESTIONS = [
    "このタスクのゴールは？",
    "期限や制約は？",
    "参考になる資料はありますか？",
...
# 受付判定の理由（デモで確実に再現できる決定的な文言。判定理由コメントの材料）
INTAKE_REASONS: dict[str, str] = {
    "hearing": "タイトルだけでは目的や制約が分からないため、先に前提を伺うのが適切です",
    "breakdown": "ひとことで終わらない大きなテーマのため、分解から始めるのが適切です",
    "execute": "作業内容が具体的なため、このまま実行AIに任せられます",
}

# 深掘り（deep_dive ask）の質問。1回目の返答だけでは分解に踏み切らない
DEEP_DIVE_QUESTION = (
