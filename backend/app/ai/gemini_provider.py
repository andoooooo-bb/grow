    ...
    *,
    ...
    if reject_reason is not None:
        ...
        parts += [
            "",
            "# 差し戻し理由（最優先で対処）",
            "前回の成果物は次の理由で差し戻されました。今回は必ずこの指摘に対処すること。",
            "対処の説明や『修正しました』等の前置きは書かず、指摘が反映された"
            "成果物本文だけを出力する。",
            f"- {reject_reason}",
        ]
    parts += [
        ...
        "# 指示",
        "- ルールに従って成果物を作成する。",
        "- 出力は成果物（完成した Markdown レポート）そのものだけにする。"
        "「承知しました」「〜します」等の返事・前置き、および"
        "「Google検索を実行します」のような作業実況は一切含めず、"
        "冒頭の3行サマリーから直接始める。",
        "- 人にしか判断できない点・レビューが必要な点に達したら、勝手に決めず、"
        "その旨を成果物末尾に「要確認事項」として明記する（前置きにはしない）。",
        "- 絵文字は使わない（※ルールに従う）。",
    ]
